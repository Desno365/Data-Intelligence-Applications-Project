...
class Ad:
    ...
    def __init__(self, ad_id: int, estimated_qualities: List[float], real_qualities: List[float], value: float, bids: List[BidsEnum]):
        assert len(estimated_qualities) == len(real_qualities)
        assert len(real_qualities) == len(bids)
    ...
    def set_bids(self, bids: List[BidsEnum]):
        assert len(bids) == len(self.real_qualities)
        self.bids = bids
        if constants.settings['adPrint']:
            print(f"New bids: {bids}")

    def set_estimated_qualities(self, estimated_qualities: List[float]):
        assert len(estimated_qualities) == len(self.real_qualities)
        self.estimated_qualities = estimated_qualities
        if constants.settings['adPrint']:
